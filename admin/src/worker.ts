...
export default {
  async fetch(request: Request, env: Env, _ctx: any): Promise<Response> {
    ...
    if (isApi) {
      ...
      // Rewrite Set-Cookie: strip Domain ONLY when it points at the backend's
      // own host (Suga) so the cookie becomes first-party on this frontend.
      // A parent-domain cookie (COOKIE_DOMAIN, ADR-051 — single session across
      // app + admin subdomains) MUST be preserved, otherwise cross-frontend
      // login breaks at the proxy. Cloudflare aggregates Set-Cookie via
      // getSetCookie() if available; fallback must not lose multiple cookies
      // (session + oauth state) on a single header.
      ...
      const targetHost = targetUrl.host.toLowerCase().replace(/^\./, '');
      for (const c of cookies) {
        const m = c.match(/[;,]\s*Domain=([^;]+)/i) ?? c.match(/^Domain=([^;]+)/i);
        const cookieDomain = m?.[1]?.trim().toLowerCase().replace(/^\./, '');
        const rewritten =
          cookieDomain && cookieDomain !== targetHost
            ? c // parent-domain session cookie — keep as-is (ADR-051)
