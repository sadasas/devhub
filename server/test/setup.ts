...
export async function resetDb(): Promise<void> {
  ...
  await pool.query(
    'TRUNCATE billing_webhook_logs, billing_package_prices, billing_packages, team_payments, mcp_keys, gcal_outbox, gcal_event_map, gcal_project_settings, gcal_connections, github_outbox, github_webhook_events, github_project_repos, github_installations, mail_outbox, email_verify_tokens, projects, users RESTART IDENTITY CASCADE',
  );
  // Seed paket default — cermin migration 021 + 043, agar tiap file test mulai konsisten.
  await pool.query(
    `INSERT INTO billing_packages (name, description, is_free, max_members, max_projects, max_storage_bytes, sort_order)
     SELECT 'Free', 'For getting started', true, 2, 3, 0, 0
     WHERE NOT EXISTS (SELECT 1 FROM billing_packages WHERE is_free)`,
  );
  await pool.query(
    `INSERT INTO billing_packages (name, description, is_free, max_members, max_projects, max_storage_bytes, sort_order)
     SELECT 'Pro', 'Unlimited members & projects', false, NULL, NULL, 104857600, 1
