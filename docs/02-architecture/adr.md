...
### ADR-048
**OAuth social login Google & GitHub — keep email+password, inbound PKCE, auto-link verified, public providers**

- **Status:** Accepted (2026-08-31) — M OAuth v0.30.0
- **Context:** Auth sejak V1 hanya email+password (`server/src/modules/auth/handlers/auth.routes.ts:15-123`, bcrypt cost 12 `password.ts:3`, JWT httpOnly `jwt.ts:14-48`). Pasar Indonesia + dev global mengharapkan `Continue with Google/GitHub`; tanpa itu friksi registrasi tinggi. Di sisi lain, sistem sudah memiliki OAuth **outbound** sebagai Authorization Server untuk MCP (`server/src/modules/oauth/oauth.routes.ts:30-323`, RFC8414/RFC9728, PKCE S256) — rentan bingung: inbound social (DevHub sebagai client ke Google/GitHub) vs outbound MCP (DevHub sebagai provider). Kebutuhan: tambah inbound tanpa menghapus custom, aman dari account-takeover via unverified email, state-CSRF, dan open-redirect; avatar sync; linking di Profile dengan guard minimal 1 method; `GET /providers` harus public tanpa login agar `AuthPage` bisa render tombol sebelum login; GitHub butuh `user:email read:user` (bukan hanya `user:email`) untuk `GET /user` + `/emails` primary verified.
- **Decision:**
  - **DB (`030_social_auth.sql`):** `users.password_hash` DROP NOT NULL (OAuth-only), `avatar_url text`, `email_verified bool DEFAULT false`, tabel `oauth_accounts(id, user_id FK CASCADE, provider CHECK google|github, provider_account_id, email, avatar_url, UNIQUE provider+account)` + indexes. ~~Personal team tetap dibuat via `WITH t` di transaksi create.~~ **Amendment:** auto-create Personal team dihapus — user baru (email maupun OAuth) mulai dengan 0 team dan buat manual via `POST /teams`.
  - **Config (`server/src/config.ts:46`):** `GOOGLE_CLIENT_ID/SECRET`, `GITHUB_CLIENT_ID/SECRET`, `OAUTH_REDIRECT_ORIGINS` (optional), `.env.example` terdokumentasi. Tidak ada `FRONTEND_URL` terpisah — `APP_PUBLIC_URL` origin dipakai untuk `frontendOrigin()`.
  - **Lib:** Tanpa `passport`/`arctic` (arctic@3 deprecated) — OAuth consumer diimplementasi manual dengan `fetch` + `node:crypto` PKCE (verifier 32B base64url, challenge SHA256), state 16B base64url. `shared/cookie.ts` ekstrak `setSessionCookie` (SameSite none/lax, Secure flag) + `setOAuthStateCookie/clearOAuthStateCookie` (httpOnly Lax 10m, payload base64url JSON `{state,codeVerifier,returnTo}`).
  - **Backend (`server/src/modules/auth/handlers/social.routes.ts`):** `GET /providers` **public** (no auth) → `{google,github}` enable check; `GET /google` & `GET /github` (rateLimit 20/15m) → generate state+PKCE, set cookie, 302 ke `accounts.google.com/o/oauth2/v2/auth` (scope `openid email profile`, `access_type offline prompt select_account`) atau `github.com/login/oauth/authorize` (scope `user:email read:user`, PKCE S256); `GET /google/callback` & `GET /github/callback` → verify `state` vs cookie + expiry, `POST /token` exchange (google `https://oauth2.googleapis.com/token`, github `https://github.com/login/oauth/access_token`), fetch profile (`google userinfo` + `verified_email`, `github /user` + `/user/emails` primary verified), branch **session-aware**: jika `verifySession(SESSION_COOKIE)` ada → **linking flow** (cek `oauth_accounts` conflict `409 ALREADY_LINKED`, insert ON CONFLICT DO NOTHING, `COALESCE avatar_url`), else **login/create flow** via `handleOAuthLogin` (1. `oauth_accounts` by provider+id → update, 2. `users` by email lower → **Opsi A auto-link hanya jika `emailVerified true`** else `409 EMAIL_NOT_VERIFIED`, 3. create user `password_hash NULL` + `oauth_accounts` dalam `withTransaction` bila verified (tanpa auto-create team — sama seperti register email). Setelah sukses `setSessionCookie` + redirect `frontendRedirect` (`returnTo` allowlist `localhost:3000/oauth/authorize` | `localhost:5173` | `https://` else `/`, error → `/?oauth_error&provider`). `GET /linked` + `DELETE /linked/:provider` (requireAuth, guard minimal 1 method: `hasPassword` atau remaining providers >0 else `400 LAST_AUTH_METHOD`).
  - **Auth guard (`auth.routes.ts`):** `POST /login` → jika `password_hash IS NULL` → `400 NO_PASSWORD` (pakai OAuth atau set password di Profile); `PATCH /password` → jika `NULL` skip `currentPassword` check (set flow); `GET /me` + `PATCH /profile` → `LEFT JOIN oauth_accounts` untuk `hasPassword`, `providers[]`, `avatarUrl`, `emailVerified`; `shared/http.ts` + `shared/cookie.ts` reuse.
