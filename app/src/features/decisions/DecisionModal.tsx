    ...
    <>
      ...
      >
        <div className="form-stack">
          {editing ? (
            <>
              <Input label={t("decisions.modal.titleLabel")} value={decision.title} onChange={(e) => update({ title: e.target.value })} />
              <div className="field-row">
                <div className="field">
                  <label className="field-label" htmlFor="decision-status">
                    {t("decisions.modal.statusLabel")}
                  </label>
                  <select id="decision-status" className="select" value={decision.status} onChange={(e) => update({ status: e.target.value as DecisionStatus })}>
                ...
                <div className="field">
                  ...
                  <input id="decision-date" className="input" type="date" value={decision.date.slice(0, 10)} onChange={(e) => update({ date: e.target.value })} />
                </div>
              </div>
              <div className="field">
                <SearchableSelect
                  ...
                  value={decision.milestoneId ?? null}
                  options={state.milestones.map((m) => ({ value: m.id, label: m.name }))}
                  onChange={(v) => update({ milestoneId: v })}
                />
              </div>
              <Textarea label={t("decisions.modal.contextLabel")} rows={3} value={decision.context} onChange={(e) => update({ context: e.target.value })} />
              <Textarea
              ...
              />
              <Textarea label={t("decisions.modal.decisionLabel")} rows={3} value={decision.decision} onChange={(e) => update({ decision: e.target.value })} />
              <Textarea label={t("decisions.modal.consequencesLabel")} rows={2} value={decision.consequences} onChange={(e) => update({ consequences: e.target.value })} />
            ...
            <>
              ...
              <DetailList>
                <DetailRow label={t("decisions.modal.statusLabel")}>
                  <Badge tone={DECISION_STATUS[decision.status].tone}>{t("decisions.status." + decision.status)}</Badge>
                </DetailRow>
                <DetailRow label={t("decisions.modal.dateLabel")}>
                  <span className="font-mono"># {decision.date.slice(0, 10)}</span>
                </DetailRow>
                <DetailRow label={t("decisions.modal.milestoneLabel", { defaultValue: "Milestone" })}>
                  {decision.milestoneId ? (state.milestones.find((m) => m.id === decision.milestoneId)?.name ?? <DetailEmpty />) : <DetailEmpty>{t("decisions.modal.noMilestone", { defaultValue: "No milestone" })}</DetailEmpty>}
                </DetailRow>
                ...
                <DetailRow label={t("decisions.modal.consequencesLabel")}>
                  {decision.consequences.trim() ? decision.consequences : <DetailEmpty>{t("decisions.modal.noConsequences")}</DetailEmpty>}
                </DetailRow>
              </DetailList>
      ...
      <ConfirmDeleteDialog open={confirmOpen} title={t("decisions.modal.deleteConfirmTitle")} description={t("decisions.modal.deleteConfirmBody")} onClose={() => setConfirmOpen(false)} onConfirm={remove} />
    </>
  );
}
