...
vi.mock("../../state/project-context", () => ({
  useProject: () => ({ state: mockState, dispatch: mockDispatch, canEdit: true, setStatus: vi.fn() }),
}));
...
describe("MilestoneModal", () => {
  ...
  it("renders edit form for milestone", () => {
    render(<MemoryRouter><MilestoneModal milestoneId={MILESTONE_ID} onClose={vi.fn()} /></MemoryRouter>);
    expect(screen.getByDisplayValue("V0.2.0")).toBeTruthy();
    expect(screen.getByDisplayValue("0.2.0")).toBeTruthy();
    expect(screen.getByDisplayValue("Shipped collaboration.")).toBeTruthy();
  });

  it("shows inputs for version and target date", () => {
    render(<MemoryRouter><MilestoneModal milestoneId={MILESTONE_ID} onClose={vi.fn()} /></MemoryRouter>);
    expect(screen.getByPlaceholderText("0.1.0")).toBeTruthy();
  });
});
